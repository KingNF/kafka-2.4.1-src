...
public class ProduceRequest extends AbstractRequest {
    ...
    // 将struct 转换成 ProduceRequest中有意义的字段，包括：partitionRecords ,partitionSize,acks,timeout,transactionID
    ...
    public static void validateRecords(short version, MemoryRecords records) {
        if (version >= 3) {
            ...
            //对于message fromat V2的版本，每一个Request 只能包含一个Batch
            if (iterator.hasNext())
                throw new InvalidRecordException("Produce requests with version " + version + " are only allowed to " +
