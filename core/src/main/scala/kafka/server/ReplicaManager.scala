...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long,
    ...
    //判断是不是有效的ack 数量，-1，1，0，如果不是的话，就不处理则request，抛出错误
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      //将topic-partition的返回状态封装成partitionResponse
      ...
      //调用 ConversionStats的callback，来更新Conversion的状态
      recordConversionStatsCallback(localProduceResults.map { case (k, v) => k -> v.info.recordConversionStats })
      //如果全部满足以下3个条件，则需要创建延迟delayedProduce，1. acks=-1, 2. 还有data需要append到log，3. 至少有一个paitition写入成功
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    def processFailedRecord(topicPartition: TopicPartition, t: Throwable) = {
      ...
      //计算一些ProduceRequest中Failed request 的比率
      ...
      //计算 produceRequest中的一些统计信息，可以不用关注
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      //如果是request中是内部topic ，并且client 不被允许的话，就报错
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        ...
        try {
          //获取对应TopicPartition的leader partition
          val partition = getPartitionOrException(topicPartition, expectLeader = true)
