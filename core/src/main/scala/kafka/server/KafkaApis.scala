...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleProduceRequest(request: RequestChannel.Request): Unit = {
    ...
    //新建的一些MAP 对象，后面的逻辑就是把他们填满
    val unauthorizedTopicResponses = mutable.Map[TopicPartition, PartitionResponse]()
    val nonExistingTopicResponses = mutable.Map[TopicPartition, PartitionResponse]()
    val invalidRequestResponses = mutable.Map[TopicPartition, PartitionResponse]()
    val authorizedRequestInfo = mutable.Map[TopicPartition, MemoryRecords]()

    //获取produceRequest发送的PartitionRecords中，producer具有写权限的Topic 列表
    ...
    //从produceRequest 中获取发送的PartitionRecords，
    for ((topicPartition, memoryRecords) <- produceRequest.partitionRecordsOrFail.asScala) {
      //如果producer 对topic没有写权限，则生成一个错误的PartitionResponse的响应
      if (!authorizedTopics.contains(topicPartition.topic))
        unauthorizedTopicResponses += topicPartition -> new PartitionResponse(Errors.TOPIC_AUTHORIZATION_FAILED)
        //如果metaDataCache中不包含该topicPartition，意味这不存在该Topic-partition，则生成一个错误的PartitionResponse的响应
      else if (!metadataCache.contains(topicPartition))
        nonExistingTopicResponses += topicPartition -> new PartitionResponse(Errors.UNKNOWN_TOPIC_OR_PARTITION)
      else
        try {
          //如果上面的权限和元数据检查都过了，那么要对于apiVersion>=3的请求，进行验证，验证包括message format，zstd等
    ...
    def sendResponseCallback(responseStatus: Map[TopicPartition, PartitionResponse]): Unit = {
      //将request中所有的Response 请求进行合并
      val mergedResponseStatus = responseStatus ++ unauthorizedTopicResponses ++ nonExistingTopicResponses ++ invalidRequestResponses
      var errorInResponse = false

      //如果ResponseStatus中存在Errors.!=NONE 的错误，则 打印Debug信息。
      ...
      //记录带宽和请求特定于配额的值，并在任何违反配额的情况下通过静音通道来限制流量。如果两个配额都被违反，请使用两个配额之间的最大限制时间。请注意，如果acks == 0，则不强制执行请求配额。
      ...
      if (produceRequest.acks == 0) {
        ...
        //当acks=0的时候，并且出现错误的时候，服务器会关闭连接，提醒client 有错误发生
        if (errorInResponse) {
