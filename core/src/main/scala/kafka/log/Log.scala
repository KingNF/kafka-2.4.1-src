...
class Log(@volatile var dir: File,
  ...
  private def append(records: MemoryRecords,
    ...
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      //对Records进行一些验证，包括：batch的尺寸，crc，同时统计最小offest，最大offest ，最大时间戳，以及最大时间戳的时间等信息到Info
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          ...
          //对Records（records 包含多个batch），进行验证，包含对magic值和压缩类型的对比检测，以及会将相对offest转换为绝对offest
          //并对offest值进行检验，
          ...
          //对validRecprds 重新赋值为检验过的Records
          ...
          // format conversion)
          //如果消息类型转换了，消息大小会发生改变，所以需要重新验证，batch大小有没有超过配置。
        ...
        //更新缓存的LeaderEpoch的Start offest
        // update the epoch cache with the epoch stamped onto the message by the leader
        validRecords.batches.asScala.foreach { batch =>
          if (batch.magic >= RecordBatch.MAGIC_VALUE_V2) {
            //v2版本才需要更新leaderEpoch的 start offset值
            maybeAssignEpochStartOffset(batch.partitionLeaderEpoch, batch.baseOffset)
          } else {
            //在某些升级的过程中，可能会发生消息格式的回退，在这种情况下，为了确保领导选举的安全性，我们将会清空leaderEpochCache，
            // 来确保截断的时候，可以按照旧版本一样进行高水位截断。
        ...
        // maybe roll the log if this segment is full，返回当前active 的 segment
        ...
        //截止到这个位置我们已经有了有效的records数据，offset也已经被分配，timestamp也已经更新，接下来，我们需要验证
        // 幂等和事务的特性，并且收集一些元数据。
  ...
  private def analyzeAndValidateRecords(records: MemoryRecords, origin: AppendOrigin): LogAppendInfo = {
    ...
    for (batch <- records.batches.asScala) {
      ...
      //记录request中的最大timestamp，和对应的offset
    ...
    //不论什么情况，都有使用broker端的压缩类型，因此这里获取目的压缩类型
